(function ($) {
  ...
  $(document).ready(function () {
    ...
    if (review.length) {
      review.owlCarousel({
        ...
        smartSpeed: 500,
        navText: [
          `Prev <img src='backEnd/img/prev.png'>`,
          `Next<img src='backEnd/img/next.png'>`,
        ],
      });
    }
  });
  ...
  if (fileInput) {
    ...
    function showFileName(event) {
      var fileInput = event.srcElement;
      var fileName = fileInput.files[0].name;
      document.getElementById("placeholderGuardiansName").placeholder =
        fileName;
    }
  }
  ...
  if (fileInput) {
    ...
    function showFileName(event) {
      var fileInput = event.srcElement;
      var fileName = fileInput.files[0].name;
      document.getElementById("placeholderFileThreeName").placeholder =
        fileName;
    }
  }
  ...
  if (fileInput) {
    ...
    function showFileName(event) {
      var fileInput = event.srcElement;
      var fileName = fileInput.files[0].name;
      document.getElementById("certificateBackgroundImage").placeholder =
        fileName;
    }
  }
    ...
    $("#select_class_subject").on("change", function () {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          $.each(data, function (i, item) {
            if (item.length) {
              $("#m_select_subject_section")
                .find("option")
                .not(":first")
                .remove();
              $("#m_select_subject_section_div ul")
                .find("li")
                .not(":first")
                .remove();
  ...
  validateAddNewroutine = () => {
    var start_time = document.getElementsByClassName(
      "start_time_required"
    ).value;
  ...
  $(document).on("click", "#create-image-option", function (event) {
    ...
    for (var i = 1; i <= number_of_option; i++) {
      ...
      appendRow +=
        "<label class='primary-btn fix-gr-bg multiple_images'><i class='fa fa-image'></i> <span class='show_file_name" +
        i +
        "'>No File Chosen [650x450]</span> <input type='file' onChange='uploadImage(" +
        i +
        ")' name='images[]' id='question_image" +
        i +
  ...
  // Update
  $(document).ready(function () {
    let methodType = $("form#add-income-update select#payment_method")
      .find(":selected")
      .data("string");
    if (methodType == "Bank") {
      $("#bankAccount").removeClass("d-none");
  ...
  //Update
  $(document).ready(function () {
    let methodType = $("form#add-expense-update select#payment_method")
      .find(":selected")
      .data("string");
    if (methodType == "Bank") {
      $("#bankAccount").removeClass("d-none");
  ...
  // Update
  $(document).ready(function () {
    let methodType = $("#edit_sell_payment_method")
      .find(":selected")
      .data("string");
    if (methodType == "Bank") {
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="mt-40 d-flex justify-content-between">
                                        <button type="button" class="primary-btn tr-bg" data-dismiss="modal">${jsLang(
                                          "cancel"
                                        )}</button>
                                        <a href="${url}" class="text-light">
                                        <button class="primary-btn fix-gr-bg" type="submit">${jsLang(
                                          "delete"
                                        )}</button>
                                            </a>
                                    </div>
